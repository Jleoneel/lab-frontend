...
export default function SettingsPage() {
  ...
  const handleSaveIva = async () => {
    ...
    if (isNaN(ivaNum) || ivaNum < 0 || ivaNum > 100) {
      await Swal.fire({
        ...
        text: "El IVA debe ser entre 0 y 100",
  ...
  const handleAddRazon = async () => {
    ...
    try {
      ...
      await Swal.fire({
        icon: "error",
        title: "Error",
        text: error.response?.data?.message || "Error al crear razón",
        confirmButtonColor: "#dc3545",
      });
    } finally {
  ...
  const handleUpdateRazon = async (id) => {
    ...
    try {
      const updated = await settingsService.updateRazon(id, {
  ...
  const handleAddCategoria = async () => {
    ...
    try {
      const res = await categoriaReactivoService.create(nuevaCategoria.trim());
  ...
  const handleUpdateCategoria = async (id) => {
    ...
    try {
      const res = await categoriaReactivoService.update(id, {
        nombre: editNombreCategoria.trim(),
      });
  ...
  const handleDeleteCategoria = async (cat) => {
    const result = await Swal.fire({
      ...
      text: `Se eliminará "${cat.nombre}". No se puede si tiene reactivos asignados.`,
      showCancelButton: true,
      confirmButtonColor: "#dc3545",
      cancelButtonColor: "#666666",
      confirmButtonText: "Eliminar",
    ...
    if (!result.isConfirmed) return;
    try {
      await categoriaReactivoService.delete(cat.id);
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "#666666" }}>
              Se aplicará a todas las nuevas cotizaciones
            </p>
          </div>
        </div>
      ...
      >
        <div className="flex items-center gap-2 mb-6">
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: "#666666" }}>
              Gestiona las razones disponibles para registrar consumos
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3 mb-4">
          ...
          />
          <button
            onClick={handleAddRazon}
            disabled={savingRazon || !nuevaRazon.trim()}
                ...
                {editingRazon === razon.id ? (
                  <div className="flex items-center gap-2 flex-1">
                    <input
                      ...
                      onKeyDown={(e) =>
                        e.key === "Enter" && handleUpdateRazon(razon.id)
                      }
                      style={{ ...inputStyle, flex: 1 }}
                      autoFocus
                      onFocus={(e) => {
                    ...
                    />
                    <button
                      onClick={() => handleUpdateRazon(razon.id)}
                  ...
                  <>
                    ...
                    <div className="flex gap-1">
                      <button
                        onClick={() => {
                          setEditingRazon(razon.id);
                          setEditNombre(razon.nombre);
                        }}
                      ...
                      <button
                        onClick={() => handleDeleteRazon(razon)}
                        className="p-2 rounded-lg transition-colors"
                        style={{ color: "#666666" }}
                        onMouseEnter={(e) => {
